...
"""
1、关于接口，从语法角度上讲，python中没有特别明显的接口概念，因为并没有提供类似 interface 这样的语法去规定什么是接口。与其谈接口，python更接近
   鸭子类型。虽然没有专门的语法，但是可以通过创建抽象基类来实现类似接口的功能。
...
4、之前自定义的类，在处理接收到参数并没有判断该参数是什么类型，而是利用了鸭子类型。比如按照鸭子类型实现namedtuple()如何接收两种类型参数的。
   如下，这里不是用 if/elif/elif 进行类型的判断，而是定义类的时候进行逻辑上的处理，让解释器进行分派不同的情况。
...
6、不要直接继承 python 的内置数据类型，如 str， dict, list, tuple 之类。因为内置类型会忽略子类覆盖的方法。如
    class MyDict(dict):
 ...
 8、写继承的原则：
      ...
      3、如果一个类仅仅是为其他类服务，与其他类之间没有直接关系，只是方便其他类来复用自己的功能，那这个类最好定义为 Mixin 类。
